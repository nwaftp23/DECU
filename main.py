...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        action='store_true',
        help="start training from pretrained network",
    )
    parser.add_argument(
        "--synset_bins",
        action='store_true',
        help="use synset_bins training",
    )
    parser.add_argument(
        "--overwrite_data_root",
        default='',
        type=str,
        help="if you want to overwrite data root in the config",
    )
    parser.add_argument(
        "-b",
        "--base",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="which component to train, controls filelist.txt model uses",
    )
    parser.add_argument(
        "--ensemble_name",
        type=str,
        default="",
        help="names folders in such a way that is common for the ensemble",
    )
    parser.add_argument(
        "-f",
        "--postfix",
...
if __name__ == "__main__":
    ...
    #print('Ensembles on the last reverse process step, that layer easier to learn other steps')
    #print('need to alter ./ldm/modules/diffusionmodules/openaimodel.py')
    #print('need gaussian parameters from last step to estimate epistemic uncertainty')
    ...
    parser = get_parser()
    print(f'seed number: parser.seed')
    parser = Trainer.add_argparse_args(parser)
    ...
    if opt.resume:
        ...
        if opt.name:
            ...
            name = ""
        nowname = now + name +'_'+ opt.ensemble_name + str(opt.seed)
    ...
    try:
        ...
        if opt.start_from_pretrained:
            ...
            if config.model['params']['cond_stage_config']['params']['lora']:
                keys_2_replace.add('cond_stage_model.embedding.weight')
            replacement_dict = {k: pretrained_dict[k] for k in keys_2_replace}
            model_dict.update(replacement_dict)
            model.load_state_dict(model_dict)
            for name, param in model.named_parameters():
                if name in keys_2_replace:
        ...
        if hasattr(model, "monitor"):
            ...
            ##NOTE: changes in order to save model every_n_train_steps
        ...
        #callbacks_cfg['image_logger']['params']['batch_frequency'] = 1
        ...
        # data
        config.data.params.train.params['comp_numb'] = opt.component
        config.data.params.train.params['synset_bins'] = opt.synset_bins
        if opt.overwrite_data_root:
